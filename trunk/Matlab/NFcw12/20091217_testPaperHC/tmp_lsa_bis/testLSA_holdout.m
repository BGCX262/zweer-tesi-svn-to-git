...
addpath(genpath('/home/turrin/matlab/Engineering'));
    ...
    if fileExist([pwd,'/',fileNameLSA]) load([pwd,'/',fileNameLSA]); end    

    tic

    onlineParam.valueToPreMultiplyFor=4;
    onlineParam.valueToAdd=1;
    onlineParam.postProcessingFunction=@onlineRatingBias;
...
% (urmConfronto)
    ...
    ls =150
    model.ls = ls;
    [positiveTests,negativeTests]=holdOut('/home/turrin/matlab/Engineering/lsaCosine', @createModel_lsaCosine, @onLineRecom_lsaCosine,urm,urmTrain,positiveTestsetReturn,negativeTestsetReturn,model,onlineParam);    
