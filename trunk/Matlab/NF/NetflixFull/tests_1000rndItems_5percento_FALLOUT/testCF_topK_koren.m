...
addpath(genpath('~/matlab/base'));
    ...
    [positiveTestsetReturn,negativeTestsetReturn]=extractTestSets (testSet,0.08,-1);
...
disp(' started sarwar unbiases');    
    ...
    fileNameRMSE = ['rmseSVDunbias25_probe_','','.mat'];
    fileNameRANK = ['rankSVDunbias25_probe_','','.mat'];
    if fileExist(fileNameRMSE) load(fileNameRMSE); end      
    if fileExist(fileNameRANK) load(fileNameRANK); end 
    ...
    onlineParamSvd.postProcessingFunction=@keep1000randomItems;
    onlineParamSvd.filterViewedItems=true;
    onlineParamSvd.bias = 2.5;
for i=1:length(lss)
    tic
    model.ls = lss(i)
    [positiveTests,negativeTests]=leaveOneOut('/home/roby/matlab/Engineering/sarwar_withVT', @createModel_sarwarWithVT, @onLineRecom_sarwarWithVT,urm,urmModel,positiveTestsetReturn,negativeTestsetReturn,model,onlineParamSvd);
    eval([ '[rmseSVDunbias25',num2str(lss(i)),']=computeRMSE(positiveTests,urm);' ]);    
    save(fileNameRMSE,'rmseSVDunbias25*');
    eval([ '[rankSVDunbias25',num2str(lss(i)),']=computeRank(positiveTests);' ]); 
    save(fileNameRANK,'rankSVDunbias25*');
    toc
end
...
%%%%%%% ITEM-ITEM_pearsonKOREN UNBIASED on originals
disp(' started ITEM-ITEM_pearsonKOREN UNBIASED on originals');

    knnn=[200];
    clear modelParam;

    fileNameRMSE = ['rmseIIpearsonKoren_unbiases_probeOnoriginals_','','.mat'];
    fileNameRANK = ['rankIIpearsonKoren_unbiases_probeOnoriginals_','','.mat'];
    if fileExist(fileNameRMSE) load(fileNameRMSE); end      
    if fileExist(fileNameRANK) load(fileNameRANK); end 
...
for i=1:length(knnn)
    tic
    modelParam.knn=knnn(i);
    [positiveTests,negativeTests]=leaveOneOut('/home/roby/matlab/Engineering/ItemItem_pearsonKoren', @createModel_pearsonIIkoren, @onLineRecom_pearsonIIkoren_unbiased,urm,urmModel,positiveTestsetReturn,negativeTestsetReturn,modelParam,onlineParamIIpearsonKoren);
    eval([ '[rmseIIpearsonKoren_unbiased',num2str(knnn(i)),']=computeRMSE(positiveTests,urm)' ]);    
    save(fileNameRMSE,'rmseIIpearsonKoren_unbiased*');
    eval([ '[rankIIpearsonKoren_unbiased',num2str(knnn(i)),']=computeRank(positiveTests);' ]); 
    save(fileNameRANK,'rankIIpearsonKoren_unbiased*');     
    toc
end
