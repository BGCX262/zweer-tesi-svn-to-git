...
Parametri.GrandezzaTest = 0.01;

% Dei tre argomenti della funzione i primi due sono obbligatori mentre il
% terzo è opzionale. Quindi controllo se ho almeno due argomenti. Se no
% ritorno e lancio l'help della funzione.
if (nargin < 2)
    help testCF_topK_koren;
    return;
end

% Aggiungo i path per le funzioni che mi servono
%addpath('/home/turrin/matlab/coldstart/fw_urmTimestamp/matlabCode');
addpath(genpath('../../../Engineering'));
addpath(genpath('../../../base'));


% Cerco nella matrice del test i valori 5...    
[a, b] = find(urmProbe == 5);
% ...e ne creo la matrice di 1 relativa
testSet = sparse(a, b, 1);

% Stampo a schermo la grandezza del test (quanti 1 ho)
display(['size testSet: ', num2str(nnz(testSet))]);

% Trovo i test positivi
[positiveTestsetReturn, negativeTestsetReturn] = extractTestSets(testSet, Parametri.GrandezzaTest, -1);

% Trovo il modello (i valori di urm che non compaiono nel probe)
...
for i=1:length(knnn)
    ...
    [positiveTests,negativeTests]=leaveOneOut('../../../Engineering/ItemItem_cosineKNN', @createModel_cosineIIknn, @onLineRecom_cosineIIknn_unbiased,urm,urmModel,positiveTestsetReturn,negativeTestsetReturn,model,onlineParamCOS);    
    eval([ '[rmseCOSunbiasedshrank',num2str(knnn(i)),']=computeRMSE(positiveTests,urm)' ]);         % trovo l'errore calcolato come sqrt((rating reale - rating calcolato)^2 / #istanze)
    save(fileNameRMSE,'rmseCOSunbiasedshrank*');
    eval([ '[rankCOSunbiasedshrank',num2str(knnn(i)),']=computeRank(positiveTests);' ]);            % trovo quanti rating son riuscito a stimare
