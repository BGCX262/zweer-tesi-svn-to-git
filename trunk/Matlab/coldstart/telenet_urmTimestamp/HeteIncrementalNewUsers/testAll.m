...
for j=1:length(days)
    ...
    if day>max(days)
	   ...
	   if ( (prev<=min(daysToJump)) & (day>=min(daysToJump)) )	
        ...
        URM_AtBtCtDt = generateURMfromTimestampFull(urm,datestr(addtodate(datenum('01/03/2008','dd/mm/yyyy'),day,'day'),'dd/mm/yyyy')); %A(t)+B(t)+C(t)+D(t)

	   URM_AtBt = generateURMfromTimestampCompactUsersFull(urm,datestr(addtodate(datenum('01/03/2008','dd/mm/yyyy'),day,'day'),'dd/mm/yyyy'),datestr(addtodate(datenum('01/03/2008','dd/mm/yyyy'),prev,'day'),'dd/mm/yyyy')); %A(t)+B(t)
	   URM_CtDt = spones(URM_AtBtCtDt)-spones(URM_AtBt);    %C(t)+D(t)
	   URM_At = generateURMfromTimestampCompactUsersFull(urm,datestr(addtodate(datenum('01/03/2008','dd/mm/yyyy'),day,'day'),'dd/mm/yyyy'),datestr(addtodate(datenum('01/03/2008','dd/mm/yyyy'),prev,'day'),'dd/mm/yyyy')); %A(t)
	   URM_AtCt = generateURMfromTimestampCompactFull(urm,datestr(addtodate(datenum('01/03/2008','dd/mm/yyyy'),day,'day'),'dd/mm/yyyy'),datestr(addtodate(datenum('01/03/2008','dd/mm/yyyy'),prev,'day'),'dd/mm/yyyy')); %A(t)+C(t)
	   URM_Ct=spones(URM_AtCt)-spones(URM_At);    %C(t)

   j,prev,day

    tic;
    [positiveTestset,negativeTestset]=extractTestSets (URM_Ct,0.1,-1);
