...
for j=1:length(days)
    ...
    if day>max(days)
	   ...
	   if ( (prev<=min(daysToJump)) & (day>=min(daysToJump)) )	
        ...
        actualURM = generateURMfromTimestampCompactUsersFull(urm,datestr(addtodate(datenum('01/03/2008','dd/mm/yyyy'),day,'day'),'dd/mm/yyyy'),datestr(addtodate(datenum('01/03/2008','dd/mm/yyyy'),prev,'day'),'dd/mm/yyyy')); %A(t)+B(t)
	   URM_At = generateURMfromTimestampCompactUsersItemsFull(urm,datestr(addtodate(datenum('01/03/2008','dd/mm/yyyy'),day,'day'),'dd/mm/yyyy'),datestr(addtodate(datenum('01/03/2008','dd/mm/yyyy'),prev,'day'),'dd/mm/yyyy')); %A(t)

   j,prev,day

    tic;
    diffURM=spones(URM_At)-spones(trainURM);    %A(t) \ A(t_0)
    [positiveTestset,negativeTestset]=extractTestSets (diffURM,0.05,-1);
