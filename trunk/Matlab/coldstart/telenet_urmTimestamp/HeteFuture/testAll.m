...
for j=1:length(days)
    ...
    if ( (prev>=max(daysToJump)) & (day<=max(daysToJump)) )	
    ...
    trainURM = generateURMfromTimestampFull(urm,datestr(addtodate(datenum('01/09/2008','dd/mm/yyyy'),-prev,'day'),'dd/mm/yyyy')); %A(t_0)
    actualURM = generateURMfromTimestampCompactUsersItemsFull(urm,datestr(addtodate(datenum('01/09/2008','dd/mm/yyyy'),-day,'day'),'dd/mm/yyyy'),datestr(addtodate(datenum('01/09/2008','dd/mm/yyyy'),-prev,'day'),'dd/mm/yyyy')); %A(t) 
    
   j,prev,day

    tic;
    diffURM=spones(actualURM)-spones(trainURM);    %A(t) \ A(t_0)
    [positiveTestset,negativeTestset]=extractTestSets (diffURM,0.1,-1);
