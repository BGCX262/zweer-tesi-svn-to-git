function [] = testAll (days,numDays,urm,daysToJump)
...
days=days(randsample(length(days),length(days)));
...
for j=1:length(days)
    ...
    if day>max(days)
	   ...
	   if ( (prev<=min(daysToJump)) & (day>=min(daysToJump)) )	
        ...
        trainURM = generateURMfromTimestampFull(urm,datestr(addtodate(datenum('07/04/2008','dd/mm/yyyy'),prev,'day'),'dd/mm/yyyy')); %A(t_0)
        URM_AtBtCtDt = generateURMfromTimestampFull(urm,datestr(addtodate(datenum('07/04/2008','dd/mm/yyyy'),day,'day'),'dd/mm/yyyy')); %A(t)+B(t)+C(t)+D(t)

	   URM_AtBt = generateURMfromTimestampCompactUsersFull(urm,datestr(addtodate(datenum('07/04/2008','dd/mm/yyyy'),day,'day'),'dd/mm/yyyy'),datestr(addtodate(datenum('07/04/2008','dd/mm/yyyy'),prev,'day'),'dd/mm/yyyy')); %A(t)+B(t)
	   URM_CtDt = spones(URM_AtBtCtDt)-spones(URM_AtBt);    %C(t)+D(t)
	   URM_At = generateURMfromTimestampCompactUsersFull(urm,datestr(addtodate(datenum('07/04/2008','dd/mm/yyyy'),day,'day'),'dd/mm/yyyy'),datestr(addtodate(datenum('07/04/2008','dd/mm/yyyy'),prev,'day'),'dd/mm/yyyy')); %A(t)
	   URM_AtCt = generateURMfromTimestampCompactFull(urm,datestr(addtodate(datenum('07/04/2008','dd/mm/yyyy'),day,'day'),'dd/mm/yyyy'),datestr(addtodate(datenum('07/04/2008','dd/mm/yyyy'),prev,'day'),'dd/mm/yyyy')); %A(t)+C(t)
	   URM_Ct=spones(URM_AtCt)-spones(URM_At);    %C(t)

   j,prev,day

    tic;
    [positiveTestset,negativeTestset]=extractTestSets (URM_Ct,1,-1);
    save(strcat('testSet_',num2str(prev)), 'positiveTestset','negativeTestset');
    
    [positiveTests,negativeTests]=leaveOneOut('/home/turrin/matlab/Engineering/topRated', @createModel_toprated, @onLineRecom_toprated,URM_CtDt,trainURM,positiveTestset,negativeTestset,1,1);
    ...
    model.ls = 15;
    [positiveTests,negativeTests]=leaveOneOut('/home/turrin/matlab/Engineering/sarwar', @createModel_sarwar, @onLineRecom_sarwar,URM_CtDt,trainURM,positiveTestset,negativeTestset,model,1);
...
%    save('recallSVD100','recallSVD100'); 
    ...
    model.knn=10;
    [positiveTests,negativeTests]=leaveOneOut('/home/turrin/matlab/Engineering/ItemItem_cosineKNN', @createModel_cosineIIknn, @onLineRecom_cosineIIknn,URM_CtDt,trainURM,positiveTestset,negativeTestset,model,1);
