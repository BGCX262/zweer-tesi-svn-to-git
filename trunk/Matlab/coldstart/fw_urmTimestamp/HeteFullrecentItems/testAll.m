function [] = testAll (refday,days,urm,testSetNumDays,append)
...
for i=days;
    ...
    if(i-testSetNumDays<0)
       continue; 
    end
    
    actualURM = generateURMfromTimestampFull(urm,datestr(addtodate(datenum(refday,'dd/mm/yyyy'),+i,'day'),'dd/mm/yyyy'));
    pastURM = generateURMfromTimestampFull(urm,datestr(addtodate(datenum(refday,'dd/mm/yyyy'),+i-testSetNumDays,'day'),'dd/mm/yyyy'));
    testURM = spones(actualURM)-spones(pastURM);
    
    % crea testSet
    [tmp1,tmp2,positiveTestsetReturn,negativeTestsetReturn]=kfolder(folds,'/home/turrin/matlab/Engineering/topRated', 0, 0,testURM,1,1,0.3,-1);
    display('created test sets');
