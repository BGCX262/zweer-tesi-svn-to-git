...
for j=1:length(days)
    ...
    if day>max(days)
	   ...
	   if ( (prev<=min(daysToJump)) & (day>=min(daysToJump)) )	
        ...
        actualURM = generateURMfromTimestampCompactUsersFull(urm,datestr(addtodate(datenum('07/04/2008','dd/mm/yyyy'),day,'day'),'dd/mm/yyyy'),datestr(addtodate(datenum('07/04/2008','dd/mm/yyyy'),prev,'day'),'dd/mm/yyyy')); %A(t)+B(t)
	   URM_At = generateURMfromTimestampCompactUsersItemsFull(urm,datestr(addtodate(datenum('07/04/2008','dd/mm/yyyy'),day,'day'),'dd/mm/yyyy'),datestr(addtodate(datenum('07/04/2008','dd/mm/yyyy'),prev,'day'),'dd/mm/yyyy')); %A(t)

   j,prev,day

    tic;
    diffURM=spones(URM_At)-spones(trainURM);    %A(t) \ A(t_0)
    [positiveTestset,negativeTestset]=extractTestSets (diffURM,0.2,-1);
    save(strcat('testSet_',num2str(prev)), 'positiveTestset','negativeTestset');
    
    [positiveTests,negativeTests]=leaveOneOut('/home/turrin/matlab/Engineering/topRated', @createModel_toprated, @onLineRecom_toprated,actualURM,trainURM,positiveTestset,negativeTestset,1,1);
    ...
    model.ls = 15;
    [positiveTests,negativeTests]=leaveOneOut('/home/turrin/matlab/Engineering/sarwar', @createModel_sarwar, @onLineRecom_sarwar,actualURM,trainURM,positiveTestset,negativeTestset,model,1);
...
%    save('recallSVD100','recallSVD100'); 
    ...
    model.knn=10;
    [positiveTests,negativeTests]=leaveOneOut('/home/turrin/matlab/Engineering/ItemItem_cosineKNN', @createModel_cosineIIknn, @onLineRecom_cosineIIknn,actualURM,trainURM,positiveTestset,negativeTestset,model,1);
