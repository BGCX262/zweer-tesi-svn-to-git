...
% msm_to_hb('prova3.',A,[],'formato per SVDLIBC','KEY','RRA',1,2);
%
%*****************************************************************************
%
...
%
  ...
  tmpVector=zeros(n,1);
  for j = 1 : n
    tmpVector(j)=length(find(A(:,j)));
  end
  sizeToPreallocate=ceil((mean(tmpVector)))*n;
  ja=zeros(1,n+1);
  ia=zeros(1,sizeToPreallocate);
  a=zeros(1,sizeToPreallocate);
  ...
  for j = 1 : n  
    ...
    if (mod(j,round(n/10))==0)
        remainingTime=(toc/j)*(n-j);        
        disp([num2str(j),'/',num2str(n), ' - remainingTime for task=',num2str(round(remainingTime)),' sec']);                
    end
  end
  ia(k+1:end)=[];
  a(k+1:end)=[];
