...
if (exist('percentageNeg', 'var')==0)
   percentageNeg=1; 
end
if (exist('percentagePos', 'var')==0)
   percentagePos=1; 
end

% Estrae solo i test positivi
if (percentageNeg < 0)
    % Trovo dove la matrice è diversa da zero
    [posI, posJ] = find(urm);
    indexPos = length(posI); % Numero delle celle != 0
    numPos = ceil(indexPos * percentagePos); % Quanti valori devo ritornare
    posIndex = randsample(indexPos,numPos); % numPos valori a caso su indexPos casi
    % Memorizzo i valori a caso e i negativi (0)
